...
(let* ((upscale 3)
       ...
       (interval-labels-gr (gr (mapcar (lambda (yy text)
                                       ...
                                       size-list
                                       interval-labels)))
       (interval-dots-gr (gr (mapcar (lambda (yy)
                                       (ln (pt (- interval-label-x 1) (* 1/2 yy))
                                           (pt (- interval-label-x 3) (* 1/2 yy))
                                           ))
                                     (cons 0 (subseq size-list 0 (length interval-labels))))))
       (btikz (make-backend-tikz :filename "vicentino-map-19.tex"))
       (btikzl (make-backend-tikz :filename "vicentino-map-19-legend.tex"))
       )
  (draw-with-multiple-backends (list btikz) (list ticks-gr tick-labels-gr
                                                  arcs-gr
                                                  tangents-gr
                                                  ...
                                                  interval-labels-gr))
  (compile-tikz btikz)
  (draw-with-multiple-backends (list btikzl) (list interval-labels-gr
                                                   interval-dots-gr))
  (compile-tikz btikzl)
  )
...
(let* ((upscale 10)
       ...
       (btikzl (make-backend-tikz :filename "vicentino-map-31-sesto-legend.tex"))
       )
  (draw-with-multiple-backends (list btikz) (list ticks-gr tick-labels-gr
                                                  arcs-gr
                                                  tangents-gr
...
(let* ((upscale 1)
       ...
       (arcs-gr (gr (do ((remainder tick-positions (rest remainder))
                         (result nil))
                        ((null remainder) (progn (setf size-list (remove-similar size-list 0.1))
                                                 result))
                      (loop for el in (rest remainder)
                            do (let* ((a (pt (first remainder) 0))
                                      (b (pt el 0)))
